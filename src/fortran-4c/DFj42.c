...
//	     binom_(integer *, integer *);
	...
	for (i__ = 0; i__ <= i__2; ++i__) {
		...
		for (m = 0; m <= i__4; ++m) {
			...
			i__6 = *k - j - i__ - l - m;
			i__7 = *k - j - i__ - l - m - ie;
			sum4 += boost_choose(i__6, ie) * dtk_(n2, r2, a2, b2, x, &
				ie) * dbsj_(nj, x, v, &i__7);
/* L25: */
		    }
		    i__5 = *k - j - i__ - l;
		    sum3 += boost_choose(i__5, m) * dtk_(n1, r1, a1, b1, x, &m) *
			    sum4;
/* L20: */
		}
		i__4 = *k - j - i__;
		sum2 += boost_choose(i__4, l) * dg_(a2, b2, x, &nn2, &l) * sum3;
/* L15: */
	    }
	    i__3 = *k - j;
	    sum1 += boost_choose(i__3, i__) * dg_(a1, b1, x, &nn1, &i__) * sum2;
/* L10: */
	}
	ret_val += boost_choose(*k, j) * dx_(nx, x, &j) * sum1;
/* L5: */
    }
    return ret_val;
} /* dfj_ */
